...
把一张用户照片制作成一张完整的诗意明信片：上方保留原摄影，下方只提取其中一个主元素进行艺术转绘，并配以克制的标题、短注。

这不是绑定某一个模型的提示词，也不要求一次生成多张候选。它是一套可安装、可直接复制、可降级运行、可二次定义的跨 Agent 工作流。

## 效果图展示

以下六组授权原图与明信片效果，展示不同题材的主体提取、外围晕染、字体选择与下方独立排版。点击每张下方的“查看原图”可对照内容取舍；完整案例记录见 [examples/README.md](examples/README.md)。
...
- 用户可调整尺寸、比例、转绘风格、排版、文字、纸张与配色倾向。
- 上方照片、下方转绘、纸张、文字与排版必须在同一次生图中完整生成，不允许后期拼接或程序排字。
...
```text
使用 xiaowai-photo-poetic-postcard，把这张照片制作成 4:5 竖版明信片。上方照片占 60%，下方改成淡墨加彩铅，标题写“海风经过”。最终只给我一张图。
```
...
每个文件都是可以单独执行的 **15 节绘图规范**：包含原图的双重角色、主体选择与删除判断、不同题材的取舍、上下分区、裁切冲突、媒介细节、尺度留白、纸底晕染、地点文案、文字布局、自由调整、能力适配、检查修正和最终交付。不需要填写模板，也不需要让 Agent 回到首页读取其他文件。
...
5. 生成前核对实际生效的要求；用户选择无文字或其他语言时，相应默认检查同步改变。
...
| 字体搭配 | Agent 根据画面内容、情绪与转绘媒介主动选择，不固定宋体 | 手写体、楷体、圆体、宋体或黑体等；清楚可读优先，用户可指定 |
| 色彩 | 从原图取色、低饱和 | 更暖、更冷、单色强调，但仍可追溯到原图 |
| 排版 | 先排主体，再根据负空间放文字 | 文字可在下半区四角间变化，不遮挡主体 |

## 不建议改掉的核心原则

自动文案优先常用字和自然短句，减少无谓的复杂字；已确认地名和用户给定原文仍须准确保留。字体选择是默认的主动判断，不需要用户先要求“换字体”，也不是按题材机械套表或随机轮换。
